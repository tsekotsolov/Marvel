...
const withScroll = WrappedComponent => {
  return class extends React.Component {
    ...
    handleScroll = () => {
      ...
      const lastElementRendered = document.querySelector(
        "div.wrapper > div:last-child"
      );
      if (lastElementRendered) {
        const lastElementRenderedOffset =
          lastElementRendered.offsetTop + lastElementRendered.clientHeight;
    ...
    filterArrayByImages = array => {
      return array.data.results.filter(
        character => !character.thumbnail.path.endsWith("not_available")
      );
    };
